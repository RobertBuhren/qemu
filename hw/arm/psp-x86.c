...
static void psp_x86_init(Object *obj) {
    ...
    object_initialize_child(obj, "x86_misc", &s->psp_x86_misc,
                            sizeof(s->psp_x86_misc), TYPE_PSP_MISC,
                            &error_abort, NULL);

    object_property_set_uint(OBJECT(&s->psp_x86_misc), (1UL << 48),
                             "psp_misc_msize", &error_abort);

    object_property_set_str(OBJECT(&s->psp_x86_misc), "X86 MEM",
                                "psp_misc_ident", &error_abort);

    object_property_set_bool(OBJECT(&s->psp_x86_misc), true, "realized",
                             &error_abort);
}
